...
func (c *Config) Configuration() error {
	...
	tokenExpired, _ := strconv.Atoi(os.Getenv("TOKEN_EXPIRE"))

	c.TokenConfig = TokenConfig{
		IssuerName:    os.Getenv("TOKEN_ISSUE"),
		SignatureKey:  []byte(os.Getenv("TOKEN_SECRET")),
		SigningMethod: jwt.SigningMethodHS256,
		ExpiresTime:   time.Duration(tokenExpired) * time.Minute,
	}

	if c.Host == "" || c.DbPort == "" || c.DbUser == "" || c.DbPassword == "" ||
		c.DbName == "" || c.Driver == "" || c.IssuerName == "" ||
		len(c.SignatureKey) == 0 || c.ExpiresTime < 0 {
		return fmt.Errorf("missing environtment")
	}
	return nil
}
